...
export async function InsertTask(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    if (!organizationId) {
        return { status: 400, jsonBody: { error: "Organization ID is required in query parameters." } };
    }

    try {
        const body = await request.json() as { title: string, description?: string, dueDate?: string, priority?: string, tags?: string[] };

        if (!body || !body.title || typeof body.title !== 'string') {
            return { status: 400, jsonBody: { error: "A non-empty 'title' is required in the request body." } };
        }
